...
export default function NewTournamentPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/tournaments", {
      ...
      body: JSON.stringify({ name, description, doubleRound }),
    });
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-stone-900 border border-stone-700 rounded-xl p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-stone-300 mb-1">Tournament Name</label>
          <input
            type="text"
            value={name}
            ...
            placeholder="e.g. Spike! Magazine Open 2026"
          />
        </div>
        ...
        <label className="flex items-start gap-3 bg-stone-800 rounded-lg p-3 cursor-pointer">
          <input
            type="checkbox"
            checked={doubleRound}
          ...
          <span className="text-sm text-stone-300">
            Double round-robin
            <span className="block text-xs text-stone-500 mt-0.5">Each pair meets twice, with home/away swapped on the return leg.</span>
          </span>
        </label>
        ...
        <div className="bg-stone-800 rounded-lg p-3 text-xs text-stone-400 space-y-1">
          ...
          <ul className="list-disc ml-4 space-y-0.5">
            <li>Every team plays every other team once{doubleRound ? " (twice)" : ""}</li>
            <li>Win = 3 points, Draw = 1 point, Loss = 0 points</li>
            <li>Add teams, then generate pairings to schedule all rounds</li>
          </ul>
        </div>
